...
<body>
	...
	* 주의사항 : else 구문이 없다.
	...
	-> 내부에 c:when, c: otherwise 라는 태그만 작성 가능!!!
	
	* 주의사항 *
	바로 한단계 아래에는
	c:when,c:otherwise,공백,jsp용 주석만 작성할 수 있고 나머지 모두 에러 발생!!
   ...
   6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
	...
	<ul>
		...
		<%--접두사 fn이 추가된 EL 구문 사용
			fn(function,기능) : 문자열 , 컬렉션 관련 기능 제공하는 JSTL - EL구문
			=>JSTLEL구문
			${fn:length(길이를 따지고싶은 컬렉션명|배열명|문자열)}
		...
		<c:if test="${vs.first}">
			...
			<li>
				<ul>
					...
					<%--현재 요소 출력하는 방법은 3가지가 있음 --%>
					<%--1.var name으로 출력 --%>
					<%--현재 접근중인 객체 vs.current --%>
					<%-- nameList.[vsindex] --%>
					<li>현재 요소(var): ${name}</li>
					<li>현재 접근중인객체 : ${vs.current}</li>
					<li>현재 요소[index] : ${nameList[vs.index]}</li>
				</ul>
			</li>
